...
UVindex$Mind.cent = scale(UVindex$Mind,center=T,scale=T)
...
lm.out = lm(UVindex~Agency.cent+Exp.cent+Age+Robot+Agency.cent*Age+Exp.cent*Age+Robot*Age,
            data=UVindex)
...
pander(summary(lm(UVindex~Agency.cent,data=UVindex)), caption = "R2 for agency")

pander(summary(lm(UVindex~Mind.cent+Age+Robot+Mind.cent*Age,
                  data=UVindex)))
...
pander(summary(lm(UVindex~Exp.cent*Age,data=UVindex.old)), caption = "R2 for exp*age")
...
#Equation 2
eq2 = lm(UVindex~Robot+Agency.cent+Age.cent,
         data=UVindex)
pander(summary(eq2))
...
#Equation 2
eq2 = lm(UVindex~Mind.cent+Robot,
         data=UVindex.old)
pander(summary(eq2), caption='Is the slope of Mind significantly different from zero?')
b = eq2$coefficients[["Mind.cent"]]
var.b = (summary(eq2)$coef[,'Std. Error'][['Mind.cent']])^2
n.b = length(resid(eq2))

#Equation 3
eq3 = lm(Mind.cent~Robot,
         data=UVindex.old)
pander(summary(eq3), caption='Is the slope of Robot significantly different from zero?')
a = eq3$coefficients[["RobotKaspar Front"]]
var.a = (summary(eq3)$coef[,'Std. Error'][['RobotKaspar Front']])^2
...
pander(summary(lm(Mind.cent~Robot+Age.cent,
                  data=UVindex)), 
       caption="Step 2: Show that the causal variable is correlated with the mediator")

#Step 3:  Show that the mediator affects the outcome variable.  Use Y as the criterion variable in a regression equation and X and M as predictors (estimate and test path b).  It is not sufficient just to correlate the mediator with the outcome because the mediator and the outcome may be correlated because they are both caused by the causal variable X.  Thus, the causal variable must be controlled in establishing the effect of the mediator on the outcome.

pander(summary(lm(UVindex~Mind.cent+Robot+Age.cent,
                  data=UVindex)), 
       caption="Step 3:  Show that the mediator affects the outcome variable")
...
#Equation 2
eq2 = lm(UVindex~Mind.cent+Robot+Age.cent,
         data=UVindex)
pander(summary(eq2), caption='Is the slope of Mind significantly different from zero?')
b = eq2$coefficients[["Mind.cent"]]
var.b = (summary(eq2)$coef[,'Std. Error'][['Mind.cent']])^2
...
lm.beta(lm(UVindex~Mind.cent+Robot+Age.cent,
         data=UVindex))

#Equation 3
eq3 = lm(Mind.cent~Robot+Age.cent,
         data=UVindex)
pander(summary(eq3), caption='Is the slope of Robot significantly different from zero?')
a = eq3$coefficients[["RobotKaspar Front"]]
var.a = (summary(eq3)$coef[,'Std. Error'][['RobotKaspar Front']])^2
...
lm.beta(lm(Mind.cent~Robot+Age.cent,
         data=UVindex))
#To test for significance, the product is then divided by the standard error of the product and the ratio is compared to a standard normal distribution.
...
###Mediation - Lavaan Package
Mediation of mind
```{r mediation_mind_lavaan}
library(lavaan)

UVindex$RobotN = ifelse(UVindex$Robot=='Kaspar Back', 1, 2)

#a path is moderated by W
model <- ' 
  #direct effects
  UVindex ~ c1*RobotN
  #mediator
  ...
  ## Condition effect of X on M at specified levels of W
  ab := a1*b
  total := ab + c1
'

fit <- sem(model, data = UVindex)
summary(fit)
 
```

Mediation of agency
```{r mediation_agency_lavaan}
library(lavaan)
...
model <- ' 
  ...
  UVindex ~ c1*RobotN
  #mediator
  Agency.cent ~ a1*RobotN
  UVindex ~ b*Agency.cent
...
Replication of mediation of mind controlling for age
```{r mediation_mind_age_lavaan}
library(lavaan)

UVindex$RobotN = factor(ifelse(UVindex$Robot=='Kaspar Back', 1, 2))

#a path is moderated by W
model <- ' 
  #direct effects
  UVindex ~ c1*RobotN + c2*Age.cent
  #mediator
  Mind.cent ~ a1*RobotN + a2*Age.cent
  UVindex ~ b*Mind.cent
...
Mediation of agency controlling for age
```{r mediation_agency_age_lavaan}
library(lavaan)
...
model <- ' 
  ...
  UVindex ~ c1*RobotN + c2*Age.cent
  #mediator
  Agency.cent ~ a1*RobotN + a2*Age.cent
  UVindex ~ b*Agency.cent
...
Mediation of experience controlling for age
```{r mediation_exp_age_lavaan}
UVindex$RobotN = ifelse(UVindex$Robot=='Kaspar Back', 1, 2)
...
model <- ' 
  ...
  UVindex ~ c1*RobotN + c2*Age.cent
  #mediator
  Exp.cent ~ a1*RobotN + a2*Age.cent
  UVindex ~ b*Exp.cent
...
Mediation of Mind on UV with Age moderating Robot on Mind
```{r moderated_mediation_mind_model2}
library(lavaan)

UVindex$RobotN = ifelse(UVindex$Robot=='Kaspar Back', 1, 2)
UVindex$WX = UVindex$RobotN*UVindex$Age.cent
...
model <- ' 
  ...
  # Conditional Direct Effect of X on Y at specified levels of W
  CDEXY1 := c1 + c3*-.5
  CDEXY2 := c1 + c3*0.5
  CDEXY3 := c1 + c3*1.5
  CDEXY4 := c1 + c3*2.5
...
Mediation of Agency on UV with Age moderating Robot on Mind
```{r moderated_mediation_agency_model2}
UVindex$RobotN = ifelse(UVindex$Robot=='Kaspar Back', 1, 2)
UVindex$WX = UVindex$RobotN*UVindex$Age.cent
...
Mediation of Experience on UV with Age moderating Robot on Mind
```{r moderated_mediation_exp_model2}
UVindex$RobotN = ifelse(UVindex$Robot=='Kaspar Back', 1, 2)
UVindex$WX = UVindex$RobotN*UVindex$Age.cent
...
fit <- sem(model, data = UVindex, test = "bootstrap")
summary(fit)
 
```

Mediation of Mind on UV with Age moderating Mind on UV
```{r moderated_mediation_mind_model3}
library(lavaan)

UVindex$RobotN = ifelse(UVindex$Robot=='Kaspar Back', 1, 2)
UVindex$WM = UVindex$Mind.cent*UVindex$Age.cent

#b path is moderated by W
model <- ' 
  #direct effects
  UVindex ~ c*RobotN
  #mediator
  Mind.cent ~ a*RobotN
  UVindex ~ b1*Mind.cent
  UVindex ~ b2*Age.cent
  UVindex ~ b3*WM
...
Mediation of Agency on UV with Age moderating Mind on UV
```{r moderated_mediation_agency_model3}
UVindex$RobotN = ifelse(UVindex$Robot=='Kaspar Back', 1, 2)
UVindex$WM = UVindex$Agency.cent*UVindex$Age.cent
...
model <- ' 
  ...
  UVindex ~ c*RobotN
  #mediator
  Agency.cent ~ a*RobotN
  UVindex ~ b1*Agency.cent
  UVindex ~ b2*Age.cent
  UVindex ~ b3*WM
...
Mediation of Experience on UV with Age moderating Mind on UV
```{r moderated_mediation_exp_model3}
UVindex$RobotN = ifelse(UVindex$Robot=='Kaspar Back', 1, 2)
UVindex$WM = UVindex$Exp.cent*UVindex$Age.cent
...
```{r moderation}
UVindex$RobotN = ifelse(UVindex$Robot=='Kaspar Back', 1, 2)
UVindex$X.Mod = UVindex$RobotN*UVindex$Age.cent
...
model <- ' 
  ...
  ## Condition effect of X on Y at specified levels of M
  mod1 := b1+b3*-1
  mod2 := b1+b3*0
  mod3 := b1+b3*1
'
...
```{r moderation}
UVindex$RobotN = ifelse(UVindex$Robot=='Kaspar Back', 1, 2)
UVindex$X.Mod = UVindex$Mind.cent*UVindex$Age.cent
...
```{r moderation2}
library(rockchalk)
model = glm(UVindex~as.vector(Age.cent)*RobotN + Mind.cent,data=UVindex)
summary(model)
plotCurves(model, plotx="RobotN",modx="Age.cent", interval="confidence", modxVals="std.dev.")

```

Mediation of Mind on UV with Age moderating Robot on Mind
```{r mediated_moderation}
UVindex$RobotN = ifelse(UVindex$Robot=='Kaspar Back', 1, 2)
UVindex$X.Mod = UVindex$RobotN*UVindex$Age.cent
