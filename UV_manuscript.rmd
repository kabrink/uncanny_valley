...
plot.new()
barCenters <- barplot(means.mat, col=c(machineColor,humanColor), legend = rownames(means.mat), beside = TRUE, ylim=g.range, yaxt='n',las=1,main=title,xlab="Age (years)", cex.main = 2.5, cex.lab = responseAxis, cex.axis = responseAxis)
axis(2, at = c(0:3), labels = c('Not at all', 'A little bit', 'A medium amount', 'A lot'), cex.axis = responseAxis) #change y axis labels
...
means.mat<-matrix(c(means.back$v.back,means.front$v.front,means.nao$v.nao),ncol=4,byrow=T)-1 #combine means into one matrix
se.mat<-matrix(c(se.back$v.back,se.front$v.front,se.nao$v.nao),ncol=4,byrow=FALSE) #combine se into one matrix

colnames(means.mat) <- c('3-5','6-8','9-11','12-17') #label the columns by age group
rownames(means.mat) <- c('Machine-like','Human-like','Nao') #label the rows by robot type
#means <- as.table(means)
means.matR <- round(means.mat,2) #produce rounded values of the matrix for displaying on the plot
g.range = range(0,ceiling(max(means.mat)+1)) #determine y-axis range

plot.new()
barCenters <- barplot(means.mat, col=c(machineColor,humanColor,naoColor), legend = rownames(means.mat), beside = TRUE, ylim=g.range, yaxt='n',las=1,main=title, xlab = "Age (years)", cex.main = 2.5, cex.lab = responseAxis, cex.axis = responseAxis)
...
plot(KasparFront$AgeYears, jitter(KasparFront$UVindex), main = "Reports of Creepiness", xlab = 'Age (years)', ylab="", yaxt = 'n', col = humanColor, cex.main = titleSizeSmall, cex.axis = responseAxis, cex.lab = responseAxis)
u <- par("usr") 
rect(u[1], u[3], u[2], u[4], col = plotFill)
points(KasparFront$AgeYears, jitter(KasparFront$UVindex), col = humanColor)
axis(2, at = c(1:4), labels = c('Not at all', 'A little bit', 'A medium amount', 'A lot'), cex.axis = responseAxis)
...
#attempt to replicate adult data
pander(mediation.test(mv=UVindex.old$Exp.cent,iv=UVindex.old$Robot,dv=UVindex.old$UVindex), caption="Test of experience as a mediator with robot appearance as predictor for children older than 9")

#all children
pander(mediation.test(mv=UVindex$Exp.cent,iv=UVindex$Robot,dv=UVindex$UVindex), caption="Test of experience as a mediator for all children with robot appearance as predictor for all children")

#age as predictor
pander(mediation.test(mv=UVindex.old$Exp.cent,iv=UVindex.old$Age.cent,dv=UVindex.old$UVindex), caption="Test of experience as a mediator with age as predictor all children older than 9")

#age as predictor all children
pander(mediation.test(mv=UVindex$Exp.cent,iv=UVindex$Age.cent,dv=UVindex$UVindex), caption="Test of experience as a mediator with age as predictor all children")

#age as binary predictor all children
UVindex$Age.bin = ifelse(UVindex$AgeGroup<3,0,1)
pander(mediation.test(mv=UVindex$Exp.cent,iv=UVindex$Age.bin,dv=UVindex$UVindex), caption="Test of experience as a mediator by Age Group (<9) for all kids")

#experience becomes more crucial for older kids, some kind of test to show this?
#we have evidence that agency is a mediator for all kids
```
...
#Equation 2
eq2 = lm(UVindex~Exp.cent+Robot,data=UVindex.old)
b = eq2$coefficients["Exp.cent"]
v.b = summary(eq2)$coef[,'Std. Error']['Exp.cent']

#Equation 3
eq3 = lm(Exp.cent~Robot,data=UVindex.old)
a = eq3$coefficients["RobotKaspar Front"]
v.a = summary(eq3)$coef[,'Std. Error']['RobotKaspar Front']
...
se.ab = a^2*v.b + b^2*v.a - v.a*v.b #Product of two independent random variables (Goodman, 1960)
...
pander(summary(lm(UVindex~Robot+Age.cent,data=UVindex)), caption="Step 1: Show that the causal variable is correlated with the outcome")
...
pander(summary(lm(Exp.cent~Robot+Age.cent,data=UVindex)), caption="Step 2: Show that the causal variable is correlated with the mediator")
...
pander(summary(lm(UVindex~Exp.cent+Robot+Age.cent,data=UVindex)), caption="Step 3:  Show that the mediator affects the outcome variable")
...
#Equation 2
eq2 = lm(UVindex~Exp.cent+Robot+Age.cent,data=UVindex)
b = eq2$coefficients["Exp.cent"]
v.b = summary(eq2)$coef[,'Std. Error']['Exp.cent']

#Equation 3
eq3 = lm(Exp.cent~Robot+Age.cent,data=UVindex)
a = eq3$coefficients["RobotKaspar Front"]
v.a = summary(eq3)$coef[,'Std. Error']['RobotKaspar Front']
...
plot.new()
par(mar=c(5,5,5,5)+0.1)
plot(KasparFront$Experience, jitter(KasparFront$UVindex), main = "Mediation Plot", xlab = 'Experience', ylab="Uncanny Index", yaxt = 'n', xaxt = 'n', col = humanColor, cex.main = titleSizeSmall, cex.axis = responseAxis, cex.lab = responseAxis)
u <- par("usr") 
rect(u[1], u[3], u[2], u[4], col = plotFill)
points(KasparFront$Experience, jitter(KasparFront$UVindex), col = humanColor)
axis(1, at = c(1:4), labels = c('Not at all', 'A little bit', 'A medium amount', 'A lot'), cex.axis = responseAxis)
axis(2, at = c(1:4), labels = c('Not at all', 'A little bit', 'A medium amount', 'A lot'), cex.axis = responseAxis)
abline(lm(KasparFront$UVindex~KasparFront$Experience+KasparFront$Age), col = humanColor, lwd = lineWidth)

points(jitter(KasparBack$Experience), jitter(KasparBack$UVindex), col = machineColor)
abline(lm(KasparBack$UVindex~KasparBack$Experience+KasparBack$Age), col = machineColor, lwd = lineWidth)
...
pander(summary(lm(UVindex~Robot+Age.cent,data=UVindex.old)), caption="Step 1: Show that the causal variable is correlated with the outcome")

#Step 2: Show that the causal variable is correlated with the mediator.  Use M as the criterion variable in the regression equation and X as a predictor (estimate and test path a).  This step essentially involves treating the mediator as if it were an outcome variable.

pander(summary(lm(Exp.cent~Robot+Age.cent,data=UVindex.old)), caption="Step 2: Show that the causal variable is correlated with the mediator")

#Step 3:  Show that the mediator affects the outcome variable.  Use Y as the criterion variable in a regression equation and X and M as predictors (estimate and test path b).  It is not sufficient just to correlate the mediator with the outcome because the mediator and the outcome may be correlated because they are both caused by the causal variable X.  Thus, the causal variable must be controlled in establishing the effect of the mediator on the outcome.

pander(summary(lm(UVindex~Exp.cent+Robot+Age.cent,data=UVindex.old)), caption="Step 3:  Show that the mediator affects the outcome variable")
...
#Equation 2
eq2 = lm(UVindex~Exp.cent+Robot+Age.cent,data=UVindex.old)
b = eq2$coefficients["Exp.cent"]
v.b = summary(eq2)$coef[,'Std. Error']['Exp.cent']

#Equation 3
eq3 = lm(Exp.cent~Robot+Age.cent,data=UVindex.old)
a = eq3$coefficients["RobotKaspar Front"]
v.a = summary(eq3)$coef[,'Std. Error']['RobotKaspar Front']
...
u <- par("usr") 
rect(u[1], u[3], u[2], u[4], col = plotFill)
points(jitter(KasparFront$Experience), jitter(KasparFront$UVindex), col = humanColor)
axis(1, at = c(1:4), labels = c('Not at all', 'A little bit', 'A medium amount', 'A lot'), cex.axis = responseAxis)
axis(2, at = c(1:4), labels = c('Not at all', 'A little bit', 'A medium amount', 'A lot'), cex.axis = responseAxis)
abline(lm(KasparFront$UVindex~KasparFront$Experience+KasparFront$Age), col = humanColor, lwd = lineWidth)
...
pander(mediation.test(mv=UVindex.ag$Agency,iv=UVindex.ag$Robot,dv=UVindex.ag$UVindex), caption="Test of Agency as a mediator with robot appearance as predictor for all children")

pander(mediation.test(mv=UVindex.ag$Agency,iv=UVindex.ag$Age,dv=UVindex.ag$UVindex), caption="Test of Agency as a mediator with age as predictor for all children")
```
...
pander(summary(lm(Agency.cent~Robot+Age.cent,data=UVindex)), caption="Step 2: Show that the causal variable is correlated with the mediator")

#Step 3:  Show that the mediator affects the outcome variable.  Use Y as the criterion variable in a regression equation and X and M as predictors (estimate and test path b).  It is not sufficient just to correlate the mediator with the outcome because the mediator and the outcome may be correlated because they are both caused by the causal variable X.  Thus, the causal variable must be controlled in establishing the effect of the mediator on the outcome.

pander(summary(lm(UVindex~Agency.cent+Robot+Age.cent,data=UVindex)), caption="Step 3:  Show that the mediator affects the outcome variable")
...
#Equation 2
eq2 = lm(UVindex~Agency.cent+Robot+Age.cent,data=UVindex)
b = eq2$coefficients["Agency.cent"]
v.b = summary(eq2)$coef[,'Std. Error']['Agency.cent']

#Equation 3
eq3 = lm(Agency.cent~Robot+Age.cent,data=UVindex)
a = eq3$coefficients["RobotKaspar Front"]
v.a = summary(eq3)$coef[,'Std. Error']['RobotKaspar Front']
...
plot.new()
par(mar=c(5,5,5,5)+0.1)
plot(KasparFront$Agency, jitter(KasparFront$UVindex), main = "Reports of Creepiness", xlab = 'Agency', ylab="Uncanny Index", yaxt = 'n', xaxt = 'n', col = humanColor, cex.main = titleSizeSmall, cex.axis = responseAxis, cex.lab = responseAxis)
u <- par("usr") 
rect(u[1], u[3], u[2], u[4], col = plotFill)
points(KasparFront$Agency, jitter(KasparFront$UVindex), col = humanColor)
axis(1, at = c(1:4), labels = c('Not at all', 'A little bit', 'A medium amount', 'A lot'), cex.axis = responseAxis)
axis(2, at = c(1:4), labels = c('Not at all', 'A little bit', 'A medium amount', 'A lot'), cex.axis = responseAxis)
abline(lm(KasparFront$UVindex~KasparFront$Agency+KasparFront$Age), col = humanColor, lwd = lineWidth)

points(KasparBack$Agency, jitter(KasparBack$UVindex), col = machineColor)
abline(lm(KasparBack$UVindex~KasparBack$Agency+KasparBack$Age), col = machineColor, lwd = lineWidth)
