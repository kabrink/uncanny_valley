...
###Figure 4
```{r figure_4, fig.height=6,fig.width=6.5}
...
{par(pty="s")
  plot(jitter(UV$AgeYears),
       jitter(UV$UV2.diff),
       pch=20,
       col=ifelse(UV$Order==1,machine_color,human_color),
       ylab="Uncanniness scores",
       xlab = "Age (Years)",
       xaxt="n")
  axis(1, at = seq(3, 18, by = 3))
  lines(agevalues, p.back, col = machine_color, lwd = 3)
  lines(agevalues, p.front, col = human_color, lwd = 3)
  legend(x = 12.75,
         y = -2.25, 
         c('Machine-like','Human-like'),
         lty=c(1,1),
         lwd=c(3,3),
         col=c(machine_color,human_color),
         cex=.75)}
```

###Figure 5
```{r figure_5, fig.width=6.75,fig.height=4.5}
